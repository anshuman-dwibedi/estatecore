...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$email || !$password) {
        ...
        $admin = $db->fetchOne("SELECT * FROM admins WHERE email = ? LIMIT 1", [strtolower($email)]);
...
<title>Admin Login — EstateCore</title>
...
<style>
  ...
  .logo-icon .dc-icon { color:var(--dc-accent); }
</style>
</head>
<body>
<div class="login-wrap">
  <div class="login-card dc-animate-fade-up">
    ...
    <div class="login-logo">
      <div class="logo-icon">
        <i class="dc-icon dc-icon-home dc-icon-xl"></i>
      </div>
      <div class="dc-h2">Estate<span style="color:var(--dc-accent)">Core</span></div>
      <p class="dc-body" style="margin-top:4px">Admin Portal</p>
    </div>
    ...
    <p class="dc-caption" style="text-align:center;margin-top:16px;color:var(--dc-text-3)">
      DevCore Portfolio Suite &middot; Real Estate Platform
    </p>
  </div>
</div>
</body>
